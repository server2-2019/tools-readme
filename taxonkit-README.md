...
peak rss: 1.17 GB
```
### 4. Output in JSON format, so you can easily collapse and uncollapse taxonomy tree in modern text editor.
```
...
```
### 2. Check deleted or merged taxids
```
...
123124124
```
### 3. Filter out invalid and deleted taxids, and "replace old taxids with new ones", you may install [csvtk](https://github.com/shenwei356/csvtk)
```
...
1458427 species
```
### 5. Show lineage consisting of taxids:
```
...
1458427 cellular organisms;Bacteria;Proteobacteria;Betaproteobacteria;Burkholderiales;Comamonadaceae;Serpentinomonas;Serpentinomonas raichei
```
### 2. Default output format ("{k};{p};{c};{o};{f};{g};{s}")
```
...
1327037   Viruses                                             Caudovirales         Siphoviridae                                   Croceibacter phage P2559Y
```
### 3. Show corresponding taxids of reformated lineage (flag -t/--show-lineage-taxids)
```
$ cat lineage.txt \
    | taxonkit reformat -t \
    | cut -f 1,4 \
...
```
### 4. Use custom strings for unclassfied ranks
```
...
```
### 5. Estimate and fill missing rank with original lineage information (experimental, very useful for formating input data for LEfSe). You can change the prefix "unclassified" using flag -p/--miss-rank-repl-prefix.
```
$ cat lineage.txt \
    | taxonkit reformat -F \
    | cut -f 1,3 \
